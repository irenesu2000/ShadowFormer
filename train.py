...
######### Model ###########
    ...
    # 使用 strftime 將 datetime.datetime 對象轉換為字符串
    file_date = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    file_folder_path = os.path.join(model_dir, file_date)
    # 創建目錄
    os.makedirs(file_folder_path)
    logname = os.path.join(file_folder_path, f'{file_date}.txt')
    summary_path = os.path.join(file_folder_path, 'runs')
    os.makedirs(summary_path)
    writer = SummaryWriter(summary_path)
    #(r'D:\ShadowFormer1019\ShadowFormer\log\ShadowFormer_istd\models\new.txt','a')
    ...
    for epoch in range(start_epoch, opt.nepoch + 1):
        ...
        for i, data in enumerate(train_loader, 0): 
            ...
            torch.nn.utils.clip_grad_norm_(model_restoration.parameters(), max_norm=1.0,error_if_nonfinite=False)
            #### Evaluation ####
            if (index+1)%eval_now==0 and i>0:
                ...
                with torch.no_grad():
                    ...
                    writer.add_scalar('PSNR/validation', psnr_val_rgb, epoch)
                    if psnr_val_rgb > best_psnr:
                        ...
                        torch.save({'epoch': epoch,
                                    ...
                                    }, os.path.join(file_folder_path,"model_best.pth"))
        ...
        torch.save({'epoch': epoch, 
                    'state_dict': model_restoration.state_dict(),
                    'optimizer' : optimizer.state_dict()
                    }, os.path.join(file_folder_path,"model_latest.pth"))   

        if epoch%opt.checkpoint == 0:
            torch.save({'epoch': epoch, 
                        'state_dict': model_restoration.state_dict(),
                        'optimizer' : optimizer.state_dict()
                        }, os.path.join(file_folder_path,"model_epoch_{}.pth".format(epoch))) 
    print("Now time is : ",datetime.datetime.now().isoformat())
